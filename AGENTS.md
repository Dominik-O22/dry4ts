...
- `2`: CLI usage/configuration error — unknown format, invalid or out-of-range option value (e.g. `--threshold` outside `(0, 1]`, `--min-lines` below 1)

The JSON output shape is `{ "clusters": ClusterReport[] }`. Each cluster groups all locations that share structural similarity above the threshold, with a `score` range, `locationCount`, and `locations` array. Each location has `file`, `startLine`, `endLine`, and `nodes`.
